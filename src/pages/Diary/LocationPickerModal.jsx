...
export default function LocationPickerModal({ isOpen, onClose, onSelect, isMapLoaded }) {
  ...
  if (!isOpen) return null; // 렌더링 조건을 컴포넌트 최상단에서 처리합니다.

  // 지도를 클릭하여 위치를 선택하는 핸들러
  ...
  const handleSearch = () => {
    ...
    if (!mapRef.current || !searchTerm.trim() || !isMapLoaded) {
      if (!isMapLoaded) alert("Google Map API가 로드되지 않았습니다. 잠시 후 다시 시도하거나, API 키를 확인하세요.");
      return;
    }

    // Places API Service 초기화
    // window.google 객체는 isMapLoaded가 true일 때만 접근 가능합니다.
